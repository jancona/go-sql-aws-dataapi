// This test adapted from https://github.com/bradfitz/go-sql-test
...
import (
	...
	_ "github.com/jancona/go-sql-aws-dataapi"
)
...
var (
	...
	dataAPI Tester = &dataapiDB{}
)

const TablePrefix = "gosqltest_"

type dataapiDB struct{}

func (p *dataapiDB) RunTest(t *testing.T, fn func(params)) {
	clusterARN := os.Getenv("GOSQLTEST_CLUSTER_ARN")
	secretARN := os.Getenv("GOSQLTEST_SECRET_ARN")
	dbName := os.Getenv("GOSQLTEST_DB")
	if dbName == "" {
		dbName = "gosqltest"
	}
...
func TestBlobs_DataAPI(t *testing.T) { dataAPI.RunTest(t, testBlobs) }
...
func TestManyQueryRow_DataAPI(t *testing.T) { dataAPI.RunTest(t, testManyQueryRow) }
...
func TestTxQuery_DataAPI(t *testing.T) { dataAPI.RunTest(t, testTxQuery) }
...
func testTxQuery(t params) {
	...
	if err != nil {
		t.Logf("cannot create table "+TablePrefix+"foo: %s", err)
	}
...
func TestPreparedStmt_DataAPI(t *testing.T) { dataAPI.RunTest(t, testPreparedStmt) }
